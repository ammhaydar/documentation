...
To mark an opportunity as lost, first open the :menuselection:`CRM app`, then select an opportunity
from the pipeline by selecting on its corresponding Kanban card. This reveals that opportunity's
detail form.

Then select :guilabel:`Lost`, located at the top of the opportunity's detail form.
...
choose an existing lost reason. If none of these are appropriate, create a new one by entering
it into the :guilabel:`Lost Reason` field, then select :guilabel:`Create`.
...
.. tip::
   ...
   :guilabel:`Mark Lost` pop-up window are required. However, including this
   information can help with traceability, accountability, and reporting purposes.

When all the desired information has been entered in the :guilabel:`Mark Lost` pop-up window, select
:guilabel:`Mark as Lost`.

.. image:: lost_opportunities/lost-opps-lost-reason.png
   :alt: Lost reasons popup with sample reasons.

After selecting :guilabel:`Mark as Lost`, a red :guilabel:`Lost` banner is added to the upper-right
...
.. note::
   Marking an opportunity as lost will archive it and set its probability to 0.
...
#. Select on the reason to be edited to highlight it.
#. Change the selected lost reason by editing the :guilabel:`Description` field.
#. When finished, select :guilabel:`Save` in the upper-left corner.

To create a new lost reason:

#. Select :guilabel:`New` in the upper-left corner of the :guilabel:`Lost Reasons` page.
#. Type the new lost reason in the :guilabel:`Description` field.
#. When finished, select :guilabel:`Save`.

View lost opportunities
=======================

To retrieve lost opportunities in Odoo **CRM**, open the :menuselection:`CRM app`. On the main
:guilabel:`Pipeline` dashboard, select the :guilabel:`Search...` bar at the top of the page, then
...
Open the :guilabel:`Filters` drop-down menu by selecting the :guilabel:`🔻(triangle pointed down)`
...
the :guilabel:`Lost` filter, under the :guilabel:`Filters` column, select :guilabel:`Add Custom
Filter`, which opens an :guilabel:`Add Custom Filter` pop-up window.

On the :guilabel:`Add Custom Filter` pop-up window, select the first field and type `Lost Reason`
in the :guilabel:`Search...` bar, or scroll to search through the list to locate it. Then, in the next field, select :guilabel:`=` from the drop-down menu. Select the third field, then select a lost reason from the drop-down menu. Finally, select :guilabel:`Add`.
...
dashboard. Or, navigate to :menuselection:`CRM app --> Sales --> My Pipeline`. From here, select the
...
Select on the Kanban card of the desired lost opportunity to restore, which opens that
opportunity's detail form.

From the lost opportunity's detail form, select :guilabel:`Restore` in the upper-left corner. Doing
...
corner. This places all the opportunities from the :guilabel:`Pipeline` page in a list view.
With the list view chosen, select the checkbox to the left of each opportunity to be restored.

Once the desired opportunities have been selected, select the :guilabel:`⚙️ Actions` drop-down menu
...
.. note::
   ...
   the :guilabel:`Leads` checkbox. Then, select :guilabel:`Save`. This adds a new :guilabel:`Leads`
...
Then, select :guilabel:`Lost`, located at the top of the lead's detail form.

This opens the :guilabel:`Mark Lost` pop-up window. From the :guilabel:`Lost Reason` drop-down menu,
choose an existing lost reason. If no applicable reason is available, create a new one by entering
it into the :guilabel:`Lost Reason` field, then selecting :guilabel:`Create`.
...
To restore a lost lead, navigate to :menuselection:`CRM app --> Leads`, then select the :guilabel:`🔻
...
Then, select on the desired lost lead to restore, which opens that lead's detail form.

From the lost lead's detail form, select :guilabel:`Restore` in the upper-left corner. Doing so
...
:guilabel:`Filters` drop-down menu, then select the :guilabel:`Lost` option. Select the checkbox to
the left of each lead to be restored.

Once the desired leads have been selected, select the :guilabel:`⚙️ Actions` drop-down menu at the
